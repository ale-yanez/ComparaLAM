...
p1 <-  ggplot(NULL, aes(x=yrs)) +
  ...
  xlab('Años') + scale_x_continuous(breaks=round(seq(1978, max(yrs), by = 6),1)) +
  ylab('Desembarque (t)') + 
...
df_mflobs <- df_mflobs[-c(1:2, 4:7, 9:15, 22:24), ]
...
df_mfloest <- df_mfloest[-c(1:2, 4:7, 9:15, 22:24), ]
...
df_hflobs <- df_hflobs[-c(1:2, 4:7, 9:15, 22:24), ]
...
df_hfloest <- df_hfloest[-c(1:2, 4:7, 9:15, 22:24), ]
...
df_mcruobs <- df_mcruobs[-c(1:20, 32), ]
...
df_mcruest <- df_mcruest[-c(1:20, 32), ]
...
df_hcruobs <- df_hcruobs[-c(1:20, 32), ]
...
df_hcruest <- df_hcruest[-c(1:20, 32), ]
...
#ggexport(psel2, filename = "Fig8_2.pdf", width=6.5, height=8, dpi=300)

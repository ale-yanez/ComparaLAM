...
dir.2<-'~/Documents/ADMwork/IFOP/2019/Lama_model/Cons_2003/sur/Lamsur2003/'
dir.3<-'~/Documents/ADMwork/IFOP/2020/Lama_model/Estatus_2008/sur/Lamsur2008/'
dir.4<-'~/Documents/ADMwork/IFOP/2020/Lama_model/Estatus_2008/sur/Retrospectivo'


dir.create(file.path('~/Documents/ADMwork/IFOP/2020/Lama_model/Estatus_2008/sur/','Retrospectivo'))#crea la carpeta  nuevamente
setwd(dir.3)
file.copy(c('LAM_sur2008.std', 'LAmS.tpl', 'lamsur2008.dat', 'LAM_sur2008.rep'), dir.4)
...
dat.file    = 'lamsur2008.dat'
dat        <- lisread(paste(dir.4,dat.file, sep='/'));
names(dat) <- str_trim(names(dat), side='right')
rep        <- reptoRlist('LAM_sur2008.rep')                                        
std        <- read.table('LAM_sur2008.std',header=T,sep='',na='NA',fill=T)  
...
for(i in 1:length(retros)){
	...
	writeData(paste('LAmS','s',i,'.dat',sep=''), data.1, append=F)}
...
for(i in 1:length(retros)){
  rep            <-reptoRlist(paste('LAmSs', i, '.rep',sep=''))
...
figretro <- ggplot(matretro) + geom_line(aes(x = year, y = value, color = Periodo)) + 
  xlab('Años') + scale_x_continuous(breaks=round(seq(1978, max(years), by = 6),1)) +

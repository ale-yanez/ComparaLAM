...
out1 <- read.admb("~/Documents/ADMwork/IFOP/2020/Lama_model/Estatus_2008/sur/Lamsur2008/LAM_sur2008")
std1 <- read.table('~/Documents/ADMwork/IFOP/2020/Lama_model/Estatus_2008/sur/Lamsur2008/LAM_sur2008.std', header=T, sep="", na="NA", fill=T)
...
p11_2 <- ggplot(data = NULL, aes(x = yrs)) + 
  ...
  geom_line(aes(y = c(rep(B0,42)), colour = 'BDo', linetype = 'BDo')) +
   annotate("text", x=1980, y=9500, label="BDo") +
   annotate("text", x=1980, y=3500, label="Brms") +
...
p11_2 <- p11_2 + theme_bw() + 
  ...
  scale_x_continuous(breaks=round(seq(1978, 2020, by = 6),1))
...
p12_2 <- p12_2 + theme_bw() + 
  ...
  scale_x_continuous(breaks=round(seq(min(yrs), 2020, by = 6),1))

p12_2

p_1 <- ggarrange(p11_2, p12_2, ncol = 2, nrow = 1)
...
p13 <- ggplot(data = NULL, aes(x = yrs)) + 
  ...
  annotate("text", x=1980, y=0.9, label="Brms") +
  scale_color_manual(name = '',
...
p13 <- p13 + theme_bw() + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.text=element_text(size=8)) +
  theme(legend.position = 'none') + ylab('BD/BDrms') + xlab('Años') + 
  scale_x_continuous(breaks=round(seq(1978, 2020, by = 6),1))
...
p14 <- ggplot(data = NULL, aes(x = yrs)) + 
  ...
  annotate("text", x=2018, y=0.9, label="Frms") +
  scale_color_manual(name = '',
                     values = c('royalblue3', 'red'),
                     limits = c('actual',  'Frms'),
                     breaks = c('actual',  'Frms')) +
...
p14 <- p14 + theme_bw() + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.text=element_text(size=8)) +
  theme(legend.position = 'none') + ylab('F/Frms') + xlab('Años') + 
  scale_x_continuous(breaks=round(seq(1978, 2020, by = 6),1))
...
plot <- ggarrange(p_1, p_2, ncol = 1, nrow = 2, align = "h", common.legend = TRUE, legend = "none")

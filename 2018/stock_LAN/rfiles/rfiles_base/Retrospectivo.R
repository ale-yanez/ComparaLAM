...
dir.2<-'C:/Users/doris.bucarey/Documents/Crustaceos/CBA_LAM/2019/norte/Retrospectivo'

#_______________________________________________________________________________________________
setwd(dir.0)
unlink(dir.2,recursive=T) 
dir.create(file.path('C:/Users/doris.bucarey/Documents/Crustaceos/CBA_LAM/2019/norte/','Retrospectivo'))#crea la carpeta  nuevamente
setwd(dir.1)
file.copy(c('Lamnor1809.dat', 'lamnor1809.exe', 'lamnor1809.std', 'lamnor18.std', 'lamnor1809.tpl'),dir.2)  
#_______________________________________________________________________________________________
setwd(dir.2) 
#_______________________________________________________________________________________________
# Retro Empírico
...
std1       <- read.table('lamnor18.std',header=T,sep='',na='NA',fill=T)  
SSBt1      <- subset(std1,name=='BD')$value
Reclutas1  <- subset(std1,name=='Restim')$value
Ft1       <- subset(std1,name=='log_Fh')$value
#---------------------------------------------------------------------------
dat.file2    = 'lamnor1809.dat'
dat2        <- lisread(paste(dir.1,dat.file2, sep='/'));
names(dat2) <- str_trim(names(dat2), side='right')
rep2        <- reptoRlist('lamnor1809.rep')                                               
std2        <- read.table('lamnor1809.std',header=T,sep='',na='NA',fill=T)  
#--------------------------------------------------------------------------
years1  <- seq(1985,2017,1)                                                              
...
plot(x2, ft2, xaxp=x2_1, cex.axis=0.8, xaxs= "i", yaxs= "i", ylim=c(0,max(rt2)+0.54),
     xlim=x2_2, type="n", ylab="Mortalidad por pesca (F)", las=1, xlab="Año", cex.lab=1.1)
...
for(i in 1:length(retros)){
	...
	writeData(paste('lamnor1809','s',i,'.dat',sep=''), data.1, append=F)}
...
plot(x2,ssbt2 , type='n', xaxp=x2_1,cex.axis=0.8,xaxs= 'i',yaxs= 'i', main = 'Biomasa desovante',
     xlim=x2_2, ylim = c(0,max(ssbt2)+1.08), ylab='t x 10e3',las=1,xlab='Año',cex.lab=1.1)
for(i in 1:length(retros)){
        rep<-reptoRlist(paste('lamnor1809s',i,'.rep',sep=''))
        lines(rep$YRS,rep$BD/10^3,type="l",col=i,lwd=2)}
legend (1985,5, c('BDt','BDt-1','BDt-2','BDt-3','BDt-4'),lty=1,col=seq(1,5,1),bty='n',cex=1)

plot(x2,rt2 , type='n', xaxp=x2_1,cex.axis=0.8,xaxs= 'i',yaxs= 'i', xlim=x2_2,
     ylim = c(0,max(rt2)+0.14), ylab='n° x 10e9',las=1,xlab='Año',cex.lab=1.1, main = 'Reclutamiento')
for(i in 1:length(retros)){
        rep<-reptoRlist(paste('lamnor1809s',i,'.rep',sep=''))
        lines(rep$YRS,rep$Rech_pre_est[2,]*10^-3,type="l",col=i,lwd=2)}
legend (1985,0.6,c('Rt','Rt-1','Rt-2','Rt-3','Rt-4'),lty=1,col=seq(1,5,1),bty='n',cex=1)

plot(x2,ft2 , type='n', xaxp=x2_1,cex.axis=0.8,xaxs= 'i',yaxs= 'i', xlim=x2_2,
     ylim = c(0,max(rt2)+0.54), ylab='n° x 10e9',las=1,xlab='Año',cex.lab=1.1, main = 'Mortalidad por pesca')
for(i in 1:length(retros)){
        rep<-reptoRlist(paste('lamnor1809s',i,'.rep',sep=''))
        lines(rep$YRS,rep$Fm_Fh[2,],type="l",col=i,lwd=2)}
legend (1985,1.0,c('Ft','Ft-1','Ft-2','Ft-3','Ft-4'),lty=1,col=seq(1,5,1),bty='n',cex=1)
dev.off()


# ___________________________________________________________________________________
# ANÁLISIS DE PATRONES RETROSPECTIVOS "MOHN RHO"
# ___________________________________________________________________________________		
...
plot(years,rel.diff.ssb[,1], type = 'p', ylab = 'rel.diff.ssb', xlab = 'años' , main='Biomasa desovante', ylim = c(-1.2,1.2))
   for (i in 1:nyears){
...
plot(years,rel.diff.f[,1], type = 'p', ylab = 'rel.diff.F', xlab = 'años' , main='Mortalidad por pesca', ylim = c(-1.2,1.2))
for (i in 1:nyears){
...
plot(years,rel.diff.r[,1], type = 'p', ylab = 'rel.diff.R', xlab = 'años' , main='Reclutas', ylim = c(-1.2,1.2))
for (i in 1:nyears){

...
DATA_SECTION
 ...
 init_matrix data(1,ntime,1,13)//Yrs/desem/cpue/Bcru/ph/cv_desem/cv_CPUE/cv_Cru/cv_ph/n_mf/n_hf/n_mc/n_hc
 init_vector edades(1,nedades)
 init_vector Tallas(1,ntallas)//(MB:1985-1986 y	1993-1996;SCI:1997-2014	sindatos2013)	
...
//!! ad_comm::change_datafile_name("17.ctl");
 ...
 init_matrix parS(1,4,1,2)//Selectividad flota (valores de partida y rango)
 ...
 number log_L50fpriorch //Selectividad cuceros
 ...
 init_int    nbloques1 // bloques selectividad flota
 init_vector ybloques1(1,nbloques1)

 init_int    nbloques2 // bloques selectividad crucero
 init_vector ybloques2(1,nbloques2)

 init_int    nqbloques  //// bloques capturabilidad cpue
 init_vector yqbloques(1,nqbloques)

 init_int    nqbloques2   // bloques capturabilidad crucero
...
//_________________________________________________________
 ...
 init_number pB // Proyección de capturas ante distintos niveles de Reclutamiento (1.0 proporcional al reclutamiento medio)
...
INITIALIZATION_SECTION
  ...
  log_rango1m     log_s1priorm 

  log_L50h        log_L50fpriorh 
  log_rango1h     log_s1priorh 

  log_L50ch        log_L50fpriorch 
  log_rango1ch     log_s1priorch 

  log_L50cm        log_L50fpriorcm 
  log_rango1cm     log_s1priorcm 
  log_Mm           log_M_priorm
  log_Mh           log_M_priorh
...
// init_3darray log_parSf(1,2,1,2,1,nbloques1,optSf_fase)
 ...
 init_bounded_vector log_L50m(1,nbloques1,0.67,1.94,optSf_fase)  
 init_vector log_rango1m(1,nbloques1,optSf_fase)

 init_bounded_vector log_L50h(1,nbloques1,0.67,1.94,optSf_fase)  
 init_vector log_rango1h(1,nbloques1,optSf_fase)

 init_bounded_vector log_L50ch(1,nbloques2,0.67,1.94,optSc_fase)  
 init_vector log_rango1ch(1,nbloques2,optSc_fase)

 init_bounded_vector log_L50cm(1,nbloques2,0.67,1.94,optSc_fase)  
 init_vector log_rango1cm(1,nbloques2,optSc_fase)
...
// FLOTA
 ...
 S1(j)=1/(1+exp(-log(19)*(edades-exp(log_L50m(j)))/exp(log_rango1m(j))));//machos
 S2(j)=1/(1+exp(-log(19)*(edades-exp(log_L50h(j)))/exp(log_rango1h(j))));//hembras
 ...
 S3(j)=1/(1+exp(-log(19)*(edades-exp(log_L50cm(j)))/exp(log_rango1cm(j))));
 S4(j)=1/(1+exp(-log(19)*(edades-exp(log_L50ch(j)))/exp(log_rango1ch(j))));
